...
public class DropAndSortableBox extends Panel {
  ...
  /**
   * MarkupBox which contains all elements, in order to support ajaxupdating.
   */
  ...
  /**
   * Construtor which generates a priolist with the given elements.
   *
   * @param id       wicket id
   * @param events   eventlist with events to be displayed. list.size() should not be grater than maxItems.
   * @param maxItems number of items the priolist should hold
   * @param isActive true if draggabiliy is given
   */
  public DropAndSortableBox(String id, List<Event> events, int maxItems, boolean isActive) {
    ...
    if (events != null) {
      ...
      for (Event event : events) {
        elements[events.indexOf(event)] = new DragableElement("DropAndSortableBox.element", event, isActive);
      }
    }
    ...
    for (int i = 0; i < maxItems; i++) {
      draggablePrioListTargets.add(new DraggablePrioListTarget("DropAndSortableBox.droptarget", DropAndSortableBox.this, i,
          elements[i], isActive));
    }
   ...
   *
   * @param element  element which is dropped.
   * @param prioList list in which it was dropped.
  ...
  public void itemDropped(DragableElement element, DraggablePrioListTarget prioList, AjaxRequestTarget target) {
    ...
    if (elements[index] == null) { /* dropped on empty slot */
      if (index == 0) {
        elements[index] = element; /* dropped in empty slot && dropped in first slot. Just add element */
      } else if (elements[index - 1] != null) { /* dropped in empty slot && previous slot is occupied. add element */
        elements[index] = element;
      } else { /* dropped in empty slot && previos slot is empty. move element up. */
      ...
      for (int i = elements.length - 1; i > index; i--) { /*
                                                                 * move items down (last item is lost) until the index where the
                                                                 * element was dropped
                                                                 */
   ...
   * @param target {@link AjaxRequestTarget} for the ajax update
   */
  private void updateCell(DraggablePrioListTarget list, AjaxRequestTarget target) {
  ...
  public void removeItem(DragableElement element, AjaxRequestTarget target) {
    ...
    for (int i = 0; i < elements.length; i++) {
      if (deleted) {
        ...
        elements[i] = null;
      } else if (elements[i] != null && elements[i].getEvent().equals(element.getEvent())) {
